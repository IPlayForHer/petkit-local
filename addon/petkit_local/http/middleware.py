...
async def device_middleware(request: web.Request, handler: Handler) -> web.StreamResponse:
    ...
    only if it carries an id), ``request["api_version"]`` and
    ``request["device_type"]``. All three are optional — handlers use
    `handlers/_common.py` to resolve a device and answer sensibly when none of
    this is present.
    """
    info = None
